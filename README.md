...
The parameters used in the AutoMLConfig are experiment_timeout_minutes=30 [which limits the experiment run to 30mins], task="classification" [given that outcome is a binary classification task], primary_metric='accuracy' [to ensure that model optimises accuracy], training_data=df [which includes both the variable and outcome columns], label_column_name='y' [to specify which column contains the target labels], n_cross_validations=5 [to conduct a 5-fold cross validation for training].\
